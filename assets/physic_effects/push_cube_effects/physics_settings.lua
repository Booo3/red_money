...
function ContractBrokerHeistItem:get_dlc_name_and_color(job_tweak,...)
    ...
    local checked_mods = {
        ["Silent Assassin"] = true, ["Pirate Perfection"] = true, ["Carry Stacker Reloaded"] = true
    }
    ...
    if job_tweak.dlc then
        if job_tweak.dlc == "custom_heist" then
            if SystemInfo:distribution() == Idstring("STEAM") and XAudio and cheat_installed == false then
				log("CSH Clear!")
				dlc_color = Color("00FF00")
                dlc_name = managers.localization:to_upper_text("cn_menu_custom_heist")
			elseif SystemInfo:distribution() == Idstring("STEAM") and not XAudio then
				log("CSH Install SBLT")
				dlc_color = Color("FF4000")
				dlc_name = managers.localization:to_upper_text("cn_menu_install_superblt")
